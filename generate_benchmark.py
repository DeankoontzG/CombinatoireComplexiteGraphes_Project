...
def attempt_free_placement(grid, grid_size, mu, sigma, min_len, max_len):
    """Tente de placer un slot aléatoirement dans un espace libre. Stratégie secondaire."""
...
def attempt_cross_placement(grid, grid_size, existing_slot, mu, sigma, min_len, max_len):
    """Tente de placer un slot perpendiculaire à un slot existant. Stratégie prioritaire."""
    
    s = existing_slot
    new_orientation = "V" if s["orientation"] == "H" else "H"
    
    # 1. On identifie les points de croisement potentiels
    ...
    for p_exist in range(s["length"]):
        ...
        if grid[r_cross][c_cross] == '.':
            ...
            #Si la case est déjà croisée, on l'ignore.
    ...
    # 2. On tente de placer un slot à partir d'un point de croisement
...
# ---------------------
# 5. Détermination des slots post-création
# ---------------------
...
# ---------------------
# 7. Génération d’une instance complète
# ---------------------
...
# ---------------------
# 8. Mise en forme des données (MiniZinc)
# ---------------------
...
# ---------------------
# 9. Génération et sauvegarde du benchmark
# ---------------------
...
def generate_benchmark():
    ...
    # --------------------------------
    # --- 9.b AFFICHAGE DES STATISTIQUES 
    # ---------------------------------
